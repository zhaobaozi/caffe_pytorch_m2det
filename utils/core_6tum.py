...
def init_net(net, cfg, resume_net):    
    if cfg.model.init_net and not resume_net:
        ...
        pretrained_dict = torch.load(resume_net)
       ...
       # print state_dict
        ...
        for k, v in state_dict.items(): 
            ...
            if head == 'module.':
                ...
                name = k
            print name
            if name[:4]!="Norm" and name[:7]!="leach.7" and name[:3]!="loc"and name[:4]!="conf"and name[:9]!="deconv1.0" and :       
                new_state_dict[name] = v
        net.load_state_dict(new_state_dict,strict=False)
'''
def set_optimizer(net, cfg):
   # print(net.Conv2d-597)
    #print "hahahahha"
    loc0_params = list(map(id, net.loc1[0].parameters()))
    loc1_params = list(map(id, net.loc1[1].parameters()))
    loc2_params = list(map(id, net.loc1[2].parameters()))
    loc3_params = list(map(id, net.loc1[3].parameters()))
    loc4_params = list(map(id, net.loc1[4].parameters()))
    loc5_params = list(map(id, net.loc1[5].parameters()))
    conf0_params = list(map(id, net.conf1[0].parameters()))
    conf1_params = list(map(id, net.conf1[1].parameters()))
    conf2_params = list(map(id, net.conf1[2].parameters()))
    conf3_params = list(map(id, net.conf1[3].parameters()))
    conf4_params = list(map(id, net.conf1[4].parameters()))
    conf5_params = list(map(id, net.conf1[5].parameters()))
    #for para in list(net.parameters())[:-12]:
    #    para.requires_grad=False
    #print(len(net))
   # print "hahahah" 
    base_params = filter(lambda p: id(p) not in loc0_params+loc1_params+loc2_params+loc3_params+loc4_params+loc5_params+conf0_params+conf1_params+conf2_params+conf3_params+conf4_params+conf5_params,net.parameters())
    
    return optim.SGD([
    {'params': base_params,'lr': cfg.train_cfg.lr[0]*0.1}, {'params':net.loc1[0].parameters()},
                                  {'params': net.loc1[1].parameters(), 'lr': cfg.train_cfg.lr[0] },
                                  {'params': net.loc1[2].parameters(), 'lr': cfg.train_cfg.lr[0] },
                                  {'params': net.loc1[3].parameters(), 'lr': cfg.train_cfg.lr[0] },
                                  {'params': net.loc1[4].parameters(), 'lr': cfg.train_cfg.lr[0] },
                                  {'params': net.loc1[5].parameters(), 'lr': cfg.train_cfg.lr[0] },
                                  {'params': net.conf1[0].parameters(), 'lr': cfg.train_cfg.lr[0] },
                                  {'params': net.conf1[1].parameters(), 'lr': cfg.train_cfg.lr[0] },
                                  {'params': net.conf1[2].parameters(), 'lr': cfg.train_cfg.lr[0] },
                                  {'params': net.conf1[3].parameters(), 'lr': cfg.train_cfg.lr[0] },
                                  {'params': net.conf1[4].parameters(), 'lr': cfg.train_cfg.lr[0] },
                                  {'params': net.conf1[5].parameters(), 'lr': cfg.train_cfg.lr[0] }],lr = cfg.train_cfg.lr[0]*0.1,momentum = cfg.optimizer.momentum,weight_decay = cfg.optimizer.weight_decay)
